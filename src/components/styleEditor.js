...
const socialMediaSelectOptions = [
  ...
  (
    <SubMenu label={name}>
          <FocusableItem>Name: <input type={"text"} value={name} onChange = {(e)=>{webContext.pageCallbacks.handleNameChange(index,e.target.value)}} name = {"homePageName"} style = {{width:"90px", borderWidth:"0px 0px 1px 0px",background:"none"}} /></FocusableItem>
          <FocusableItem>Path: <input type={"text"} value={path} onChange = {(e)=>{webContext.pageCallbacks.handlePathChange(index,e.target.value)}} name = {"homePageName"} style = {{width:"90px", borderWidth:"0px 0px 1px 0px",background:"none"}} /></FocusableItem>
          <MenuItem><Link to={path}>Visit Page</Link></MenuItem>
          <MenuDivider />
          <MenuItem><a onClick={()=>{webContext.pageCallbacks.deletePage(name,index)}}>Delete Page</a></MenuItem>
    </SubMenu>
  ))
  ...
  (
    <SubMenu label={location}>
          <FocusableItem>Site: <select onChange = {(e)=>{webContext.socialMediaCallbacks.handleSocialSiteChange(index,e.target.value)}} value={location}>{socialMediaSelectOptions}</select></FocusableItem>
          <FocusableItem>link: <input type={"text"} value={link} onChange = {(e)=>{webContext.socialMediaCallbacks.handleSocialLinkChange(index,e.target.value)}} name = {"homePageName"} style = {{width:"90px", borderWidth:"0px 0px 1px 0px",background:"none"}} /></FocusableItem>
          <MenuItem><Link to={link}>Visit Link</Link></MenuItem>
          <MenuDivider />
          <MenuItem><a onClick={()=>{webContext.socialMediaCallbacks.deleteSocialMedia(location,index)}}>Delete Link</a></MenuItem>
    </SubMenu>
  ))
      ...
      <div className={"row m-auto w-100 "+(webContext.isAdminPage?"":" bg-light")} style={{zIndex:2}}>
        ...
        <div className={"col text-center "+(webContext.webStyle.isMobile?"mx-1 g-0":"mx-4")}>
          ...
          <Menu className="nav-item dropdown" menuButton={<MenuButton className={"styleEditorIcon dropdown-toggle font-shrink-md m-0"}><FontAwesomeIcon  icon={faFile} /></MenuButton>} transition>
            ...
            {pages}
            <MenuButton className={"styleEditorSubmenuIcon "} onClick = {()=>webContext.pageCallbacks.addPage()}><FontAwesomeIcon  icon={faPlus} /></MenuButton>
            <MenuDivider />
        ...
        <div className={"col text-center "+(webContext.webStyle.isMobile?"mx- g-0":"mx-4")}>
          {/* Socials Pages */}
          <Menu className="nav-item dropdown " menuButton={<MenuButton className={"styleEditorIcon dropdown-toggle font-shrink-md m-0"}><FontAwesomeIcon  icon={faTwitter} /></MenuButton>} transition>
            ...
            {socialMediaLinks}
            <MenuButton className={"styleEditorSubmenuIcon"} onClick = {()=>webContext.socialMediaCallbacks.addSocialMedia()}><FontAwesomeIcon  icon={faPlus} /></MenuButton>
          </Menu>
        </div>
        <div className={"col text-center "+(webContext.webStyle.isMobile?"mx- g-0":"mx-4")}>
          {/* Socials Pages */}
          <MenuButton className={"styleEditorIcon"} onClick = {()=>webContext.saveWebsite()}><FontAwesomeIcon  icon={faSave} /></MenuButton>

        </div>
        <div className={"col text-center "+(webContext.webStyle.isMobile?"mx- g-0":"mx-4")}>
          {/* Socials Pages */}
          <MenuButton className={"styleEditorIcon"} onClick = {()=>{webContext.toggleStyleEditor()}}><FontAwesomeIcon  icon={faTimes} /></MenuButton>
